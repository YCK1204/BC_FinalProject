...
public class SplitOnDeath : MonoBehaviour, Game.Monster.ISpecialAbillity
{
    public void Init(NormalMonster monster)
    {
        monster.OnDiedSA += () =>
        {
            ...
            NormalMonster monster1 = Instantiate(monster, monster.transform.position + Vector3.right * 0.5f + Vector3.up * 0.5f, Quaternion.identity);
           monster1.MonsterData.AddModifier(new StatModifier(Game.Monster.StatType.Attack, Game.Monster.ModifierType.Multiply, 0.5f, null));
            ...
            if (Extension.HasComponent<SplitOnDeath>(monster1.gameObject))
               monster1.OnDiedSA = null;

            NormalMonster monster2 = Instantiate(monster, monster.transform.position + Vector3.right * -0.5f + Vector3.up * 0.5f, Quaternion.identity);
